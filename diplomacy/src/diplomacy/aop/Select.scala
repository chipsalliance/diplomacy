...
import org.chipsalliance.diplomacy.nodes.{AnyMixedNode, BaseNode, MixedNode}
...
object Select {
  ...
  /** Applies the collect function to a [[LazyModule]] and its children if the filter function returns true. Stops
    * recursing when the filter function returns false. e.g. for this hierarchy A / \ B C / \ \ D E F
    *
