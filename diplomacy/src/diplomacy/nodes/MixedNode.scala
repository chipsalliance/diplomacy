...
/** [[Dangle]] captures the `IO` information of a [[LazyModule]] and which two [[BaseNode]]s the [[Edges]]/[[Bundle]]
  * connects.
  *
  * [[Dangle]]s are generated by [[BaseNode.instantiate]] using [[diplomacy.nodes.MixedNode.danglesOut]] and
  * [[diplomacy.nodes.MixedNode.danglesIn]] , [[LazyModuleImp.instantiate]] connects those that go to internal or
  * explicit IO connections in a [[LazyModule]].
  *
  * @param source
  ...
  *   `danglesIn`.
  * @param data
  *   actual [[Data]] for the hardware connection.
  */
...
/** The sealed node class in the package, all node are derived from it.
  ...
  * }}}
  */
private[diplomacy] abstract class MixedNode[DI, UI, EI, BI <: Data, DO, UO, EO, BO <: Data](
  ...
  protected[diplomacy] def instantiate(): Seq[Dangle] = {
    instantiated = true
    if (!circuitIdentity) {
      (iPorts.zip(in)).foreach { case ((_, _, p, _), (b, e)) =>
        if (p(MonitorsEnabled)) inner.monitor(b, e)
      }
    }
